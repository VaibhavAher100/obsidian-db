...
describe('DatabaseTable', () => {
  it('renders column headers', () => {
    render(<DatabaseTable rows={[]} columns={[textCol, numCol]} engine={noopEngine} onUpdate={vi.fn()} />);
  ...
  it('renders one row per Row', () => {
    ...
    render(<DatabaseTable rows={rows} columns={[textCol]} engine={noopEngine} onUpdate={vi.fn()} />);
    expect(screen.getByText('Alice')).toBeDefined();
    expect(screen.getByText('Bob')).toBeDefined();
  });

  it('DataColumn cell shows frontmatter value', () => {
    render(<DatabaseTable rows={[row({ price: 42 })]} columns={[numCol]} engine={noopEngine} onUpdate={vi.fn()} />);
  ...
  it('FormulaColumn aggregate cell shows same value in every row', () => {
    ...
    render(<DatabaseTable rows={rows} columns={[sumCol]} engine={engine} onUpdate={vi.fn()} />);
  ...
  it('FormulaColumn per-row cell shows per-row values', () => {
    ...
    render(<DatabaseTable rows={rows} columns={[ifCol]} engine={engine} onUpdate={vi.fn()} />);
  ...
  it('committing CellEditor calls onUpdate with new value', async () => {
    ...
    const rows = [row({ name: 'Alice' })];
    render(<DatabaseTable rows={rows} columns={[textCol]} engine={noopEngine} onUpdate={onUpdate} />);
    fireEvent.click(screen.getByText('Alice'));
  ...
  it('FormulaColumn cell is not clickable into edit mode', () => {
    ...
    render(<DatabaseTable rows={[row({ price: 99 })]} columns={[sumCol]} engine={engine} onUpdate={vi.fn()} />);
